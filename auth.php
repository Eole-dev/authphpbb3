...
class auth_plugin_authphpbb3 extends DokuWiki_Auth_Plugin {
	...
	private function load_configuration() {
		if ($this->use_phpbb_cache()) {
			...
			if (!@file_exists($this->_phpbb_conf['root_path'] . 'config.' . $this->_phpbb_conf['phpEx'])) {
				dbglog('authphpbb3: phpBB3 installation cannot be found.'); 
				return false;
			}
			...
			foreach (array('dbhost', 'dbname', 'dbuser', 'dbpasswd', 'table_prefix') as $member) {
				if (empty($this->_phpbb_conf[$member])) {
					dbglog("authphpbb3 error: phpBB3 config variable {$member} not set.");
					return false;
				}
			}
			if ($this->get_phpbb_url() === false) {
				dbglog("authphpbb3 error: cannot get phpBB URL.");
				return false;
			}
			if (!$this->get_phpbb_cookie_name()) {
				dbglog("authphpbb3 error: cannot get phpBB cookie name.");
				return false;
			}
	...
	*
	* @return cache	Cache of the phpBB configuration.
	*/
	private function get_phpbb_cache() {
		if ($this->_cache === null) {
			$this->_cache = new cache('authphpbb3', '.phpbb3cache');
		}
	...
	private function phpbb_connect() {
		if (!$this->_phpbb_sql_link) {
			...
			if (!$this->_phpbb_sql_link || $this->_phpbb_sql_link->connect_error) {
				dbglog('authphpbb3: cannot connect to database server (' . $this->_phpbb_sql_link->connect_errno .')');
	...
	*/
	private function get_phpbb_cookie_name() {
		...
		if (!$result) {
			dbglog('authphpbb3: database structure error.');
			return false;
		}
	...
	*/
	private function get_phpbb_user_groups() {
		...
		$result = $this->_phpbb_sql_link->query($query);
		if (!$result) {
			dbglog('authphpbb3: cannot get user\'s groups.');
			return false;
		}
	...
	*/
	private function do_login_cookie() {
        ...
        if (empty($this->_phpbb_user_session_id) || !ctype_xdigit($this->_phpbb_user_session_id)) {
			dbglog('authphpbb3: invalid SID in user\'s cookie.');
			return false;
		}
		...
		$result = $this->_phpbb_sql_link->query($query);
		if (!$result) {
			dbglog('authphpbb3: no session found in database.');
			return false;
		}
		$row = $result->fetch_object();
		if ($phpbb_cookie_user_id !== (int)$row->session_user_id) {
			dbglog('authphpbb3: invalid SID/User ID pair.');
			$result->close();
			unset($row);
			return false;
		}
		...
		if ($current_time > $this->_phpbb_sessiontime) {
			...
			$result = $this->_phpbb_sql_link->query($query);
			if (!$result) {
				dbglog('authphpbb3: cannot update session.');
			}
		}
		...
		$result = $this->_phpbb_sql_link->query($query);
		if (!$result) {
			dbglog('authphpbb3: no user found in database.');
			return false;
		}
		$row = $result->fetch_object();
	...
	*/
	public function get_phpbb_url() {
		...
		while ($row = $result->fetch_object()) {
			switch ($row->config_name) {
				...
				case 'script_path':
					...
					break;
				case 'server_port':
					$server_port = trim($row->config_value);
					break;
				default:
					break;
			}
		...
		$this->_phpbb_conf['url'] = $server_protocol . $server_name . ':' . $server_port . $script_path;
		$result->close();
		unset($row);
	...
	*/
	public function getUserData($user, $requireGroups = true) {
		...
		$cache = new cache('authphpbb3_getUserData_' . $user, '.phpbb3cache');
		$user_data = false;
		...
		if (($this->_cache_duration > 0) && $cache->useCache($depends)) {
			...
			$user_data = array(
				...
				'phpbb_profile'	=> $this->_phpbb_conf['url'] . '/memberlist.php?mode=viewprofile&u=' . $this->_phpbb_user_id,
